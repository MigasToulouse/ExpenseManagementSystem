...
IMAGE_NAME=expense-management-system
CONTAINER_NAME=expense-management-system
DOCKERFILE_PATH=.
...
testcov:
	@rm -rf htmlcov
	uv run pytest -v --cov-report html --cov-report xml --cov=src/expense_management_system tests/
...
clean:
	...
	rm -rf build
	rm -rf perf.data*
	rm -rf expense_management_system/*.so
	rm -rf .mypy_cache
	rm -rf .benchmark
	rm -rf .hypothesis
	rm -rf docs-site
